...
<link type="text/css" rel="stylesheet"
	href="<c:url value='/css/egovframework/com/cop/bbs/style.css' />">
<script
...
<body>
	...
	<div class="blog_wrap">
		...
		<div class="blog_header">
			...
			<div class="h_menu">
				<ul class="gnb">
					<c:forEach items="${blogNameList}" var="resultInfo"
						varStatus="status">
						<li><a id="titleck" href="#"><c:out
									value="${resultInfo.bbsNm}" /></a></li>
					</c:forEach>
				</ul>
				...
				<ul class="gnb r">
		            <c:choose>
		                <c:when test="${loginUserCnt == 1}">
		                    ...
		                    <li><a href="#">개인블로그관리</a></li>
		                    <li><button class="write"><spring:message code="button.create" /></button></li>
		                </c:when>
		            </c:choose>
		        </ul>
			</div>
		...
		<div class="post">
			...
			<div class="listBox">
				...
				<form name="postCnt" method="post" onChange="$('#titleck').click();">
					<div class="post_opt">
						<label for="" class="blind"><spring:message
								code="comCopBlog.articleBlogList.setListNumbers" /></label>
						<!-- 포스트 개수 설정 -->
						<select name="searchCnd" id="searchCnd">
							<option value="5"
								<c:if test="${searchVO.searchCnd == '5'}">selected="selected"</c:if>><spring:message
									code="comCopBlog.articleBlogList.listNumber5" /></option>
							<!-- 5줄 보기 -->
							<option value="10"
								<c:if test="${searchVO.searchCnd == '10'}">selected="selected"</c:if>><spring:message
									code="comCopBlog.articleBlogList.listNumber10" /></option>
							<!-- 10줄 보기 -->
							<option value="15"
								<c:if test="${searchVO.searchCnd == '15'}">selected="selected"</c:if>><spring:message
									code="comCopBlog.articleBlogList.listNumber15" /></option>
							<!-- 15줄 보기 -->
							<option value="20"
								<c:if test="${searchVO.searchCnd == '20'}">selected="selected"</c:if>><spring:message
									code="comCopBlog.articleBlogList.listNumber20" /></option>
							<!-- 20줄 보기 -->
							<option value="30"
								<c:if test="${searchVO.searchCnd == '30'}">selected="selected"</c:if>><spring:message
									code="comCopBlog.articleBlogList.listNumber30" /></option>
							<!-- 30줄 보기 -->
						</select>
					</div>
				</form>
		...
		<div id="blog_cnt" class="blog_cnt" style="display: block;">
			...
			<div class="comm_List">
				<dl>
					<dt>
						<strong>답글 #1</strong> <span>2019-01-28 11:26:49</span> <span>
							<a href="#">삭제</a>
						</span>
					</dt>
					<dd>111</dd>
				...
				<dl>
					<dt>
						<strong>답글 #2</strong><span>2019-01-28 11:26:53</span> <span>
							<a href="#">삭제</a>
						</span>
					</dt>
					<dd>222</dd>
				...
				<dl>
					<dt>
						<strong>답글 #3</strong><span>2019-01-28 11:26:56</span> <span>
							<a href="#">삭제</a>
						</span>
					</dt>
					<dd>333</dd>
					...
					<dd>
						<form id="formComment" name="formComment" method="post">
							<textarea name="commentCn"
								placeholder="댓글은 500byte 까지 작성할 수 있습니다."></textarea>
							<button type="button" onclick="#">등록</button>
							<input name="bbsId" type="hidden" value=""><input
								name="nttId" type="hidden" value=""><input name="blogId"
								type="hidden" value=""><input name="modified"
								type="hidden" value=""><input name="commentNo"
								type="hidden" value=""><input name="blogAt"
								type="hidden" value="Y">
						</form>
					</dd>
				</dl>
	...
	<!-- 블로그 //-->
	<form name="blogfrm">
		<input name="blogCn" type="hidden" value=""> <input
			name="blogName" type="hidden" value=""> <input name="bbsId"
			type="hidden" value="">
	</form>
</body>
</html>