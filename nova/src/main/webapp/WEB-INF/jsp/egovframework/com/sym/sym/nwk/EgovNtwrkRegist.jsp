...
function fncNtwrkInsert() {
    ...
    if(confirm("<spring:message code="comSymSymNwk.ntwrkRegist.validate.save"/>")){ //저장 하시겠습니까?
        if(!validateNtwrk(varFrom)){
            return;
        } else {
        	if(!ipValidate(varFrom.ntwrkIp.value, ''))
                return;
            else if(!ipValidate(varFrom.gtwy.value, '<spring:message code="comSymSymNwk.ntwrkRegist.gtwy"/>')) //게이트웨이
                return;
            else if(!ipValidate(varFrom.subnet.value, '<spring:message code="comSymSymNwk.ntwrkRegist.subnet"/>')) //서브넷
                return;
            else if(!ipValidate(varFrom.domnServer.value, '<spring:message code="comSymSymNwk.ntwrkRegist.domnServer"/>')) //DNS
                return;
            else
            	varFrom.submit();
        }
...
function ipValidate(ipValue, ipName) {
    ...
    if(IPvalue == "0.0.0.0") {
        alert(IPvalue + " : <spring:message code="comSymSymNwk.ntwrkRegist.validate.ip.notAllowed"/>"); //예외 아이피 입니다.
        result = false;
    } else if (IPvalue == "255.255.255.255") {
        alert(result =IPvalue + " : <spring:message code="comSymSymNwk.ntwrkRegist.validate.ip.notAllowed"/>"); //예외 아이피 입니다.
        result = false;
    } else {
        result = true;
    }
