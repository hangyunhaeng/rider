...
<body>
	...
	<div class="board">
		...
		<form name="items" method="post"
			action="<c:url value='/uss/ion/bnr/getBanner.do'/>">
			<h1>
				<spring:message code="ussIonBnr.bannerMainList.bannerMainList" />
			</h1>
			<span>※<spring:message
					code="ussIonBnr.bannerMainList.EgovBannerDc" /></span>
				...
				<thead>
					<tr>
						<th scope="col"><spring:message
								code="ussIonBnr.bannerMainList.bannerNm" /></th>
					...
					<c:forEach var="banner" items="${bannerList}" varStatus="status">
						<tr>
							<td class="left"><a href=""
								onclick="fncSelectBanner('<c:out value="${banner.bannerId}"/>');return false;"><c:out
										value="${banner.bannerNm}" /></a></td>
							<td><input class="btn01" type="button" value="수정"
								onclick="fncSelectBanner('<c:out value="${banner.bannerId}"/>');return false;" />
							</td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<input type="hidden" name="bannerId" value=""> <input
				type="hidden" name="pageIndex" value="1"> <input
				type="hidden" name="searchCondition" value="1"> <input
