...
<div class="sort_area">
	...
	<div class="date_view" style="left:39%;">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year-1%>&amp;month=<%=month%>">prev</a></li>
			<li class="date"><%=year%> <spring:message code="comCopSmtSim.Navi.Year"/></li><!-- 년 -->
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year+1%>&amp;month=<%=month%>">next</a></li>
			<li class="date">&nbsp;</li>

			<%if(month > 0 ){ %>
			<li><a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year%>&month=<%=month-1%>" title="<spring:message code="comCopSmtSim.goPrevMonth"/>" class="prev">prev</a></li><!-- 이전달로 가기 -->
			<%}%>
			<li class="date"><%=month+1%> <spring:message code="comCopSmtSim.Navi.Month"/></li><!-- 월 -->
			<%if(month < 11 ){ %>
			<li><a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageMonthList.do' />?year=<%=year%>&month=<%=month+1%>" class="next" title="<spring:message code="comCopSmtSim.goNextMonth"/>">next</a></li><!-- 다음달로 가기 -->
			<%}%>
		</ul>
	</div>
</div>
...
<table class="month">
	<caption><spring:message code="comCopSmtSim.Gbn.Daily"/> <spring:message code="comCopSmtSim.title"/></caption> <!-- 월간 부서일정관리 -->
	<colgroup>
	...
	<thead>
		<tr>
			<th><spring:message code="comCopSmtSim.Monthly.sunday"/></th> <!-- 일 -->
			<th><spring:message code="comCopSmtSim.Monthly.monday"/></th> <!-- 월 -->
			<th><spring:message code="comCopSmtSim.Monthly.tuesday"/></th> <!-- 화 -->
			<th><spring:message code="comCopSmtSim.Monthly.wednesday"/></th> <!-- 수 -->
			<th><spring:message code="comCopSmtSim.Monthly.thursday"/></th> <!-- 목 -->
			<th><spring:message code="comCopSmtSim.Monthly.friday"/></th> <!-- 금 -->
			<th><spring:message code="comCopSmtSim.Monthly.saturday"/></th> <!-- 토 -->
		</tr>
	</thead>
<TBODY>
<TR>
...
for(int index = 1; index <= endDay; index++)
{
	...
	out.println("<a href=\"JavaScript:fn_egov_regist_IndvdlSchdulManage('"+iUseDate+"');\"><font color='"+color+"'>"+index+"</font></a>");
	out.println("<BR>");
		...
		for(int i=0;i < listResult.size(); i++){
			...
			if(iUseDate >= iBeginDate && iUseDate <= iBeginEnd){
				...
				if(iSchdulWeekFirest == 1
					...
					out.print("<div class='divWeekBar' style='margin-top:"+(iDayCount*20)+"px;position:absolute;width:"+iDotTextWidth+"px;border:solid 0px;background-color:"+ egovMap.get("schdulWeekColor") +";'><a href=\"JavaScript:fn_egov_detail_IndvdlSchdulManage('" + (String)egovMap.get("schdulId") + "')\" class='linkWeek'>");
					out.print((String)egovMap.get("schdulNm"));
					out.println("</a></div>");
				}
				//일별스케줄개수
