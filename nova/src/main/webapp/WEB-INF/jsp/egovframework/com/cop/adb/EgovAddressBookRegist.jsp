...
<script type="text/javascript">
	...
	function fn_egov_regist_adbkInf(form){
	...
	function fn_egov_inqire_user(){
		...
		var retVal;
		var url = "<c:url value='/cop/adb/openPopup.do?requestUrl=/cop/adb/selectManList.do&width=750&height=800'/>";
		var openParam = "dialogWidth: 850px; dialogHeight: 750px; resizable: 0, scroll: 1, center: 1";
...
<form name="adbk" action="${pageContext.request.contextPath}/cop/adb/RegistAdbkInf.do"  method="post" onSubmit="fn_egov_regist_adbkInf(document.forms[0]); return false;">
...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr>
			...
			<td class="left">
			   <input name="adbkUser" id="adbkUser" size="40" readonly maxlength="255" style="width:94%;" />
		 ...
		 <c:forEach var="result" items="${searchVO.adbkMan}" varStatus="status">
		  <tr>
			...
			<td>
				...
				<!-- </form>  -->
			     <!-- 2016.08.16 신 UI 적용 -->
 				<button class="btn_s2" onClick="javascript:fn_egov_deleteUser'<c:out value="${result.emplyrId}" />'); return false;" title="<spring:message code="button.select" /> <spring:message code="input.button" />"><spring:message code="button.select" /></button>
			...
			<td>
			    ...
			    <span class="button">
			     ...
			     <!-- </form> -->
			     <!-- 2016.08.16 신 UI 적용 -->
 				<button class="btn_s2" onClick="javascript:fn_egov_deleteUser('<c:out value="${result.ncrdId}" />'); return false;" title="<spring:message code="button.select" /> <spring:message code="input.button" />"><spring:message code="button.select" /></button>
