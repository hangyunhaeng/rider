...
<title><spring:message
		code="ussIonWikBmk.wikiBookmarkList.wikiBookmarkList" /></title>
<!-- 위키북마크 목록 -->
...
<script type="text/javaScript">
	...
	function fn_egov_delCnt_WikiBookmark() {
		...
		//undefined
		if (FLength == 1) {
			if (document.listFormS.checkList.checked == true) {
				g_nDelCount++;
			}
		} else {
			for (var i = 0; i < FLength; i++) {
...
.TopMenuBg {
	background-image: url('<c:url value='/ images/ egovframework/ com/ cmm/ uss/ umt/
		menu_bg.jpg '/>');
...
.table_listA th {
	...
	font-weight: bold;
	color: #4f657d;
	background: url('<c:url value='/ images/ egovframework/ com/ cmm/ uss/ umt/
		bg_list_th.gif '/>');
}
...
.page_prevend {
	...
	top: 3px;
	background-color: transparent;
	background-image: url('<c:url value='/ images/ egovframework/ com/ cmm/ uss/ umt/ icon/
		icon_prevend.gif '/>');
	border-style: none;
}
...
.btnBackground {
	background-image: url('<c:url value='/ images/ egovframework/ com/ cmm/ uss/ umt/ button/
		bu2_bg.gif '/>');
}
</style>
...
<body>
	...
	<div class="board">
		<form name="listFormS"
			action="<c:url value='/uss/ion/wik/bmk/listWikiBookmark.do'/>"
			method="post">
			<h1>
				<spring:message
					code="ussIonWikBmk.wikiBookmarkList.wikiBookmarkList" />
			</h1>
			<!-- 위키북마크 목록 -->
			<span><spring:message
					code="ussIonWikBmk.wikiBookmarkList.bookmarkgd" /></span>
			<!-- 위키북마크 등록은 JSP WIki 설치가 필요합니다.(자세한 내용은 위키가이드 참고) -->
			...
			<div class="search_box"
				...
				<ul>
					<li><label for=""><spring:message
								code="ussIonWikBmk.wikiBookmarkList.bookmarkNm" /> : </label> <!-- 북마크명 -->
						<input class="s_input2 vat" name="searchKeyword" type="text"
						value="<c:out value='${searchKeyword}'/>" size="10" maxlength="35"
						onkeypress="press();"
						onkeyup="if(window.event.keyCode==13){fn_egov_search_WikiBookmark(); return false;}"
						title="검색어" /> <input class="s_btn" type="submit"
						value='<spring:message code="button.inquire" />'
						title='<spring:message code="button.inquire" />'
						onclick="fn_egov_search_WikiBookmark(); return false;" /> <input
						class="s_btn" type="submit"
						value="<spring:message code="button.delete"/>"
						title="<spring:message code="button.delete"/>"
						onclick="fn_egov_delete_WikiBookmark(); return false;" /> <!-- 삭제 --></li>
				</ul>
			</div>
				...
				<thead>
					<tr>
						<th scope="col"><input type="checkbox" name="checkAll"
							id="checkAll" title="전체선택" value="1"
							onclick="fn_egov_checkAll_WikiBookmark();"></th>
						<th scope="col"><spring:message code="table.num" /></th>
						<!-- 순번 -->
						<th scope="col"><spring:message
								code="ussIonWikBmk.wikiBookmarkList.bookmarkNm" /></th>
						<!-- 북마크명 -->
					</tr>
				</thead>
				<tbody>
					...
					<c:if test="${fn:length(resultList) == 0}">
						<tr>
							<td colspan="3"><spring:message code="common.nodata.msg" />
							</td>
						</tr>
					</c:if>
					<%-- 데이터를 화면에 출력해준다 --%>
					<c:forEach items="${resultList}" var="resultInfo"
						...
						<tr>
							<td><input type="checkbox" name="checkList" title="선택"
								value="${resultInfo.wikiBkmkId}" /></td>
							<td><c:out
									value="${(searchVO.pageIndex-1) * searchVO.pageSize + status.count}" /></td>
							<td style="">
								<div style="visibility: hidden; display: none;">
									<a href="#LINK_PAGE${status.count}"></a>
								</div> <a
								href="<c:url value="../../../../../JSPWiki/Wiki.jsp"><c:param name="page" value="${resultInfo.wikiBkmkNm}"/></c:url>"
								target="_blank"><c:out value="${resultInfo.wikiBkmkNm}" /></a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<input name="cmd" type="hidden" value=""> <input
				name="searchCondition" type="hidden" value="A.WIKI_BKMK_NM">
