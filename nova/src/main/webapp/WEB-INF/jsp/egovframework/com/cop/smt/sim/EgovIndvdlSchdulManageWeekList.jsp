...
List<List<String>> listWeekGrop = (List<List<String>>) request.getAttribute("listWeekGrop");
...
listWeek = (ArrayList<String>)listWeekGrop.get(iNowWeek);
...
<head>
<HEAD>
	<TITLE><spring:message code="comCopSmtSim.Gbn.Weekly"/> <spring:message code="comCopSmtSim.title"/></TITLE>
...
<div class="sort_area">
	...
	<div class="date_view">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageWeekList.do' />?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>">prev</a></li>
			<li class="date"><%=iNowYear%> <spring:message code="comCopSmtSim.Navi.Year"/></li>
			...
			<%if(iNowMonth < 11 ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageWeekList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth+1%>&amp;week=<%=0%>">next</a></li>
			<%}%>
			<li class="date">&nbsp;</li>
			<%if(iNowWeek > 0 ){ %>
			<li><a class="prev" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageWeekList.do' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek-1%>">prev</a></li>
			<%}%>
			<li class="date"><%=iNowWeek+1%> <spring:message code="comCopSmtSim.Navi.Week"/> </li><!-- 주 -->
			<%if(iNowWeek < listWeekGrop.size()-1 ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageWeekList.do' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek+1%>">next</a></li>
			<%}%>
...
<table class="title">
<caption><spring:message code="comCopSmtSim.Gbn.Weekly"/> <spring:message code="comCopSmtSim.title"/></caption> <!-- 주간 부서일정관리 -->
<colgroup>
...
<thead>
<tr>
	<th><spring:message code="comCopSmtSim.Weekly.date"/></th> <!-- 날짜 -->
  ...
  <tr>
    ...
    <td valign="middle" height="50px">
	    <c:if test="${not empty listResult}">
	        <c:forEach var="egovMap" items="${listResult}">
	            <c:set var="schdulBgnde" value="${egovMap.schdulBgnde}" />
	            <c:set var="schdulEndde" value="${egovMap.schdulEndde}" />
	            <c:set var="iBeginDate" value="${fn:substring(schdulBgnde, 0, 8)}" />
	            <c:set var="iEndDate" value="${fn:substring(schdulEndde, 0, 8)}" />

	            <c:if test="${iUseDate >= iBeginDate and iUseDate <= iEndDate}">
	                <table>
	                    <tr>
	                        <td nowrap>
	                            <div>
	                                <a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDetail.do' />?schdulId=${egovMap.schdulId}" target="_parent">
	                                    ${fn:substring(schdulBgnde, 8, 10)}시
	                                    ${fn:substring(schdulBgnde, 10, 12)}분<br/>~
	                                    ${fn:substring(schdulEndde, 8, 10)}시
	                                    ${fn:substring(schdulEndde, 10, 12)}분
	                                </a>
	                            </div>
	                        </td>
	                    </tr>
