...
<c:set var="pageTitle">
	<spring:message code="comUssOlhQna.qnaVO.title" />
</c:set>
<!DOCTYPE html>
<html>
<head>
...
<body onload="fn_egov_init()">
	...
	<form name="qnaForm"
		action="<c:url value='/uss/olh/qna/selectQnaList.do'/>" method="post"
		onSubmit="fn_egov_search_qna(); return false;">
		<div class="board">
			<h1>${pageTitle}
			...
			<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
				<ul>
						...
						title="<spring:message code="title.searchCondition" /> <spring:message code="input.cSelect" />">
							...
							<option value="1"
								<c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if>><spring:message
									code="comUssOlhQna.qnaVO.qestnSj" /></option>
							<!-- 질문제목 -->
					</select></li>
					<!-- 검색키워드 및 조회버튼 -->
					<li><input class="s_input" name="searchWrd" type="text"
						...
						<span class="btn_b"><a
							href="<c:url value='/uss/olh/qna/insertQnaView.do' />"
		...
		<thead>
			<tr>
				...
				<th class="board_th_link"><spring:message
						code="comUssOlhQna.qnaVO.qestnSj" /></th>
				<!-- 질문제목 -->
				<th><spring:message code="table.reger" /></th>
				<!-- 작성자 -->
				<th><spring:message
						code="comUssOlhQna.qnaVO.qnaProcessSttusCode" /></th>
				<!-- 진행상태 -->
				...
				<tr>
					...
					<td>
						<form name="subForm" method="post"
							action="<c:url value='/uss/olh/qna/selectQnaDetail.do'/>">
							<input name="qaId" type="hidden"
								value="<c:out value="${resultInfo.qaId}"/>"> <input
								name="pageIndex" type="hidden"
								value="<c:out value='${searchVO.pageIndex}'/>" /> <span
								class="link"><input type="submit"
								value="<c:out value='${fn:substring(resultInfo.qestnSj, 0, 40)}'/>"
