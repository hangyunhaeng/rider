...
<title><spring:message
		code="ussOlpMgt.meetingManageLisEmpLyrPopup.userPopup" /></title>
<!-- 사용자 검색 팝업 -->
...
<script type="text/javascript"
	src="<c:url value='/js/egovframework/com/cmm/showModalDialogCallee.js'/>"></script>
<script type="text/javaScript">
	/* ********************************************************
	 * 페이징 처리 함수
	 ******************************************************** */
	...
	function fn_egov_open_Popup(cnt, esntlId) {
		...
		if (opener[1] == "typeMeeting") {
			...
			opener[0].document.getElementById("mnaerNm").value = document
					.getElementById("iptText_" + cnt).value;
		} else if (opener[1] == "typeDeptSchdule") {
			opener[0].document.getElementById("schdulChargerId").value = esntlId;
			opener[0].document.getElementById("schdulChargerName").value = document
					.getElementById("iptText_" + cnt).value;
		}
...
<body style="margin-top: 10px">
	...
	<div class="board">
		<form name="listForm" id="listForm"
			action="<c:url value='/uss/olp/mgt/EgovMeetingManageLisEmpLyrPopup.do'/>"
			method="post">
			<h1>
				<spring:message
					code="ussOlpMgt.meetingManageLisEmpLyrPopup.searchId" />
			</h1>
			<!-- 아이디 검색 -->
			...
			<div class="search_box"
				...
				<ul>
					<li><select name="searchCondition" class="select"
						title="<spring:message code="select.searchCondition"/>">
							<!-- 검색조건선택 -->
							<option value=''>--
							...
							<option value='USER_NM'
								<c:if test="${searchCondition == 'USER_NM'}">selected</c:if>><spring:message
									code="ussOlpMgt.meetingManageLisEmpLyrPopup.userNm" /></option>
							<!-- 이름 -->
							<option value='EMPLYR_ID'
								<c:if test="${searchCondition == 'EMPLYR_ID'}">selected</c:if>><spring:message
									code="ussOlpMgt.meetingManageLisEmpLyrPopup.emplyrId" /></option>
							<!-- 아이디 -->
							<option value='OFFM_TELNO'
								<c:if test="${searchCondition == 'OFFM_TELNO'}">selected</c:if>><spring:message
									code="ussOlpMgt.meetingManageLisEmpLyrPopup.offmTelno" /></option>
							<!-- 전화번호 -->
					</select> <input class="s_input2 vat" name="searchKeyword" type="text"
						value="${searchKeyword}" size="10"
						title="<spring:message code="input.input"/>" /> <!-- 검색단어입력 --> <input
						class="s_btn" type="submit"
						value="<spring:message code="button.inquire" />"
						title="<spring:message code="button.inquire" />"
						onclick="fn_egov_search_MeetingManage(); return false;" /></li>
				</ul>
			</div>
				...
				<thead>
					<tr>
						...
						<th scope="col"><spring:message
								code="ussOlpMgt.meetingManageLisEmpLyrPopup.emplyrId" /></th>
						<!-- 아이디 -->
						<th scope="col"><spring:message
								code="ussOlpMgt.meetingManageLisEmpLyrPopup.userNm" /></th>
						<!-- 이름 -->
						<th scope="col"><spring:message
								code="ussOlpMgt.meetingManageLisEmpLyrPopup.offmTelno" /></th>
						<!-- 전화번호 -->
						<th scope="col"><spring:message
								code="ussOlpMgt.meetingManageLisEmpLyrPopup.detailAdres" /></th>
						<!-- 주소 -->
						<th scope="col"></th>
					</tr>
				</thead>
				<tbody>
					...
					<c:forEach items="${resultList}" var="resultInfo"
						...
						<tr>
							...
							<td class="left">${resultInfo.homeadres}
								${resultInfo.detailAdres}</td>
							<td><input class="btn01" type="button"
								value="<spring:message code="input.cSelect"/>"
								onclick="fn_egov_open_Popup('${status.count}', '${resultInfo.esntlId}')" />
								<input name="iptText_${status.count}"
								id="iptText_${status.count}" type="hidden"
								value="${resultInfo.emplyrId}"></td>
						</tr>
					</c:forEach>
