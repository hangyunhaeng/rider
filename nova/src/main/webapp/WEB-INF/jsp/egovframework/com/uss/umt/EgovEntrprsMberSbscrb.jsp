...
<c:set var="pageTitle">
	<spring:message code="comUssUmt.entrprsUserManage.title" />
</c:set>
<!DOCTYPE html>
<html>
<head>
...
<validator:javascript formName="entrprsManageVO"
...
<script type="text/javaScript" defer="defer">
	...
	function fn_modal_setting() {
		...
		$("#egovModal")
				.setEgovModalTitle(
						"<spring:message code="comUssUmt.userManageRegistModal.title" />"); //아이디 중복 확인
		var content = "";
		content = content
					...
					success : function(returnData, status) {
						if (status == "success") {
							if (returnData.usedCnt > 0) {
								...
								$("#divModalResult")
										.html(
												"<font color='red'><spring:message code="comUssUmt.userManageRegistModal.result" /> : ["
														+ returnData.checkId
														+ "]<spring:message code="comUssUmt.userManageRegistModal.useMsg" /></font>");
							} else {
								//사용가능한 아이디입니다.
								$("#divModalResult")
										.html(
												"<font color='blue'><spring:message code="comUssUmt.userManageRegistModal.result" /> : ["
														+ returnData.checkId
														+ "]<spring:message code="comUssUmt.userManageRegistModal.notUseMsg" /></font>");
							}
						} else {
							alert("ERROR!");
							return;
...
<body onload="fn_egov_init()">
	...
	<!-- content start -->
	<form:form modelAttribute="entrprsManageVO"
		action="${pageContext.request.contextPath}/uss/umt/EgovEntrprsMberSbscrb.do"
		name="entrprsManageVO" method="post"
		onSubmit="fnInsert(document.forms[0]); return false;">

		<!-- 우편번호검색 -->
		<input type="hidden" name="zip_url"
			value="<c:url value='/sym/ccm/zip/EgovCcmZipSearchPopup.do'/>" />
		<div class="wTableFrm">
			<!-- 타이틀 -->
			<h2>${pageTitle}
			...
			<table class="wTable"
				...
				<tbody>
					...
					<!-- 기업회원아이디 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.id" />
					</c:set>
					<tr>
						<th><label for="entrprsmberId">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left"><form:input path="entrprsmberId"
								id="entrprsmberId" title="${title} ${inputTxt}" size="20"
								readonly="true" maxlength="20" style="width:80%;" />
							<button id="btnMbrId" class="btn_s2" onClick="return false;"
								...
								<spring:message
									code="comUssUmt.entrprsUserManageRegistBtn.idSearch" />
							</button>
							<div>
					...
					<!-- 회사명 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.name" />
					</c:set>
					<tr>
						<th><label for="cmpnyNm">${title}</label> <span class="pilsu">*</span></th>
						<td class="left"><form:input path="cmpnyNm"
					...
					<!-- 비밀번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.pass" />
					</c:set>
					<tr>
						<th><label for="entrprsMberPassword">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left">
							<div>
								<form:password path="entrprsMberPassword"
					...
					<c:set var="title">
						<spring:message
							code="comUssUmt.entrprsUserManageRegist.passConfirm" />
					</c:set>
					<tr>
						<th><label for="entrprsMberPassword2">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left"><input name="entrprsMberPassword2"
							id="entrprsMberPassword2" title="${title} ${inputTxt}"
							type="password" size="50" maxlength="20" /></td>
					</tr>
					<!-- 비밀번호힌트 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.passHit" />
					</c:set>
					<tr>
						<th><label for="entrprsMberPasswordHint">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left"><form:select path="entrprsMberPasswordHint"
								id="entrprsMberPasswordHint" title="${title} ${inputSelect}">
					...
					<!-- 비밀번호정답 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.passOk" />
					</c:set>
					<tr>
						<th><label for="entrprsMberPasswordCnsr">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left"><form:input path="entrprsMberPasswordCnsr"
								id="entrprsMberPasswordCnsr" title="${title} ${inputTxt}"
					...
					<c:set var="title">
						<spring:message
							code="comUssUmt.entrprsUserManageRegist.indutyCode" />
					</c:set>
					<tr>
						<th><label for="indutyCode">${title}</label></th>
						<td class="left"><form:select path="indutyCode"
								id="indutyCode" title="${title} ${inputSelect}">
								<form:option value="" label="${inputSelect}" />
								<form:options items="${indutyCode_result}" itemValue="code"
									itemLabel="codeNm" />
							</form:select></td>
					</tr>
					<!-- 기업구분코드 -->
					<c:set var="title">
						<spring:message
							code="comUssUmt.entrprsUserManageRegist.entrprsSeCode" />
					</c:set>
					<tr>
						<th><label for="entrprsSeCode">${title}</label></th>
						<td class="left"><form:select path="entrprsSeCode"
								id="entrprsSeCode" title="${title} ${inputSelect}">
								<form:option value="" label="${inputSelect}" />
								<form:options items="${entrprsSeCode_result}" itemValue="code"
									itemLabel="codeNm" />
							</form:select></td>
					</tr>
					...
					<!-- 대표이사이름 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.cxfc" />
					</c:set>
					<tr>
						<th><label for="cxfc">${title}</label></th>
						<td class="left"><form:input path="cxfc" id="cxfc"
					...
					<!-- 사업자등록번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.bizrno" />
					</c:set>
					<tr>
						<th><label for="bizrno">${title}</label> <span class="pilsu">*</span></th>
						<td class="left"><form:input path="bizrno" id="bizrno"
								title="${title} ${inputTxt}" cssClass="txaIpUmt" size="20"
								maxlength="15" />
							<div>
					...
					<!-- 법인등록번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.jurirno" />
					</c:set>
					<tr>
						<th><label for="jurirno">${title}</label></th>
						<td class="left"><form:input path="jurirno" id="jurirno"
								title="${title} ${inputTxt}" cssClass="txaIpUmt" size="20"
								maxlength="15" />
							<div>
					...
					<!-- 회사전화번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.tel" />
					</c:set>
					<tr>
						<th><label for="areaNo">${title}</label> <span class="pilsu">*</span></th>
						<td class="left"><form:input path="areaNo" id="areaNo"
								title="전화번호" size="5" maxlength="5" style="width:40px;" /> - <form:input
								path="entrprsMiddleTelno" id="entrprsMiddleTelno" size="5"
								maxlength="5" style="width:40px;" /> - <form:input
								path="entrprsEndTelno" id="entrprsEndTelno" size="5"
					...
					<!-- 팩스번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.fax" />
					</c:set>
					<tr>
						<th><label for="">${title}</label></th>
						<td class="left"><form:input path="fxnum" id="fxnum"
								title="${title} ${inputTxt}" cssClass="txaIpUmt" size="20"
								maxlength="15" />
							<div>
					...
					<!-- 우번번호 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.post" />
					</c:set>
					<tr>
						<th><label for="zip">${title}</label> <span class="pilsu">*</span></th>
						<td class="left"><form:input path="zip"
								title="${title} ${inputTxt}" type="text" size="20" value=""
								maxlength="8" style="width:60px;" /> <!-- form:hidden path="zip" id="zip" -->
					...
					<!-- 주소 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageRegist.addr" />
					</c:set>
					<tr>
					...
					<c:set var="title">
						<spring:message
							code="comUssUmt.entrprsUserManageRegist.addrDetail" />
					</c:set>
					<tr>
					...
					<!-- 신청자이름 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageList.regName" />
					</c:set>
					<tr>
						<th><label for="">${title}</label> <span class="pilsu">*</span></th>
						<td class="left"><form:input path="applcntNm" id="applcntNm"
								title="${title} ${inputTxt}" cssClass="txaIpUmt" size="20"
								maxlength="20" />
							<div>
					...
					<!-- 신청자이메일주소 -->
					<c:set var="title">
						<spring:message code="comUssUmt.entrprsUserManageList.regMail" />
					</c:set>
					<tr>
						<th><label for="applcntEmailAdres">${title}</label> <span
							class="pilsu">*</span></th>
						<td class="left"><form:input path="applcntEmailAdres"
								id="applcntEmailAdres" title="${title} ${inputTxt}"
								cssClass="txaIpUmt" size="30" maxlength="50" />
							<div>
								<form:errors path="applcntEmailAdres" cssClass="error" />
							</div> <input type="hidden" name="entrprsMberSttus" value="DEFAULT" />
						</td>
					</tr>
				</tbody>
			</table>
