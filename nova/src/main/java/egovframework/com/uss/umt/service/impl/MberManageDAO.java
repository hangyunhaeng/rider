...
public class MberManageDAO extends EgovComAbstractDAO{
    ...
    /**
     * 일반회원 암호수정
     * @param passVO 기업회원수정정보(비밀번호), 사용자 본인이 직접 비밀번호를 변경(본인이 확인했음)을 저정한다
     */
    public void updatePasswordSelf(MberManageVO passVO) {
        update("mberManageDAO.updatePassword_Self", passVO);
    }
    ...
    /**
     *  COMTNGNRLMBER mber_id로 조회
     * @param mberManageVO
     * @return
     */
    public MberManageVO selectUserListRider(UserDefaultVO userDefaultVO) {
    	return selectOne("mberManageDAO.selectUserListRider", userDefaultVO);
    }
