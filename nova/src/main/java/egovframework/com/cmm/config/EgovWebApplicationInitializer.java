...
import egovframework.com.rd.MemberDivisionFilter;
...
public class EgovWebApplicationInitializer implements WebApplicationInitializer {
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
        ...
        // ResourcePatternResolver를 사용하여 context-*.xml 파일들 로드
        PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver();
        Resource[] resources = null;
		try {
			resources = resolver.getResources("classpath*:egovframework/spring/com/**/context-*.xml");
		} catch (IOException e) {
        ...
        for (Resource resource : resources) {
            ...
            // 파일 이름에 "context-scheduling"이 포함된 파일을 제외
            if (fileName != null /*&& !fileName.contains("context-scheduling")*/) {
                try {
					configLocations.add(resource.getURI().toString());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
        ...
        // 필터링된 파일 목록을 설정
        rootContext.setConfigLocations(configLocations.toArray(new String[0]));
        rootContext.refresh();
        rootContext.start();

        // ContextLoaderListener를 추가하여 Spring 컨텍스트를 관리
		...
		if("security".equals(EgovProperties.getProperty("Globals.Auth").trim())) {
			...
			//로그인 실패시 반활 될 URL설정
			egovSpringSecurityLoginFilter.setInitParameter("loginURL", "/com/com0003.do");
			//로그인 처리 URL설정
			egovSpringSecurityLoginFilter.setInitParameter("loginProcessURL", "/com/com0004.do");
			//처리 Url Pattern
		...
		//-------------------------------------------------------------
		// 한균행 : 관리자와 사용자를 구별할 필터 추가
		//-------------------------------------------------------------
		FilterRegistration.Dynamic springSecurityFilterChain = servletContext.addFilter("MemberDivisionFilter", new MemberDivisionFilter());
		springSecurityFilterChain.addMappingForUrlPatterns(null, false, "/*");
